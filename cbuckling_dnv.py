...
class dnv(buckling_common):
	...
	# eq.(6.5)
	def resistance_syRd(fy, Psd):
		syR = resistance_syR(fy, Psd)
		print("syR =" + str(syR))
		syRd = syR/gm
		return syRd

	# eq.(6.6)
	def resistance_syR(fy, Psd):
		EM = Basic.ElasticModulus(material)
		kp = reduction_factor_kp(fy, Psd)
		k = kappa(fy)
...
def kappa(fy):
	...
	if Lc <= 0.2:
		...
		k = 1 / (2 * mth.pow(Lc,2)) + 0.07
		
	return k

def reduction_factor_kp(fy, Psd):
